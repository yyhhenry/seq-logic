...
export const remarkId = (storage: DiagramStorage) => {
  ...
  const nodes = Object.fromEntries(
    Object.entries(storage.nodes).map(
      ([id, node]) => [_NNA(nodeIdMapping.get(id)), node] satisfies [string, Node]
    )
  );
...
export class Diagram {
  ...
  parse() {
    ...
    for (const [_id, wire] of this.wires.entries()) {
      ...
      _NNA(this.status.get(end)).powered ||= _NNA(this.status.get(start)).powered;
    }
    for (const id of this.nodes.keys()) {
      this.getGroupRoot(id);
    }
    this.updatePrec = new Map([...this.nodes.entries()].map(([id]) => [id, new Set()]));
    this.updateSucc = new Map([...this.nodes.entries()].map(([id]) => [id, new Set()]));
    for (const [_id, wire] of this.wires.entries()) {
      if (!wire.not) {
        continue;
      }
